  ...
  return (
    <section id="connect" className="md:flex">
      <div className="basis-1/2 flex-grow-0 flex-shrink-0 py-12 px-8 md:px-24">
        {heading && <h2 className="text-orange uppercase text-[5rem]">{heading}</h2>}
        {text && <p className="uppercase text-[1.8rem] mt-2 mb-7">{text}</p>}
        {menu.length > 0 && (
          <nav className="flex flex-col">
            {menu.map((item: ItemProps & { link: { slug: { current: string } } }) => (
              <NavItem className="font-bold leading-[2] block !text-black !text-[1.6rem] normal-case" key={item._key} {...item} />
            ))}
          </nav>
        )}
      </div>
      <div className="basis-1/2 flex-grow-0 flex-shrink-0 md:flex md:justify-center md:items-center relative bg-decoOrange">
        ...
        <ul className="h-[200px] flex items-center justify-center w-full top-0 relative z-10">
          {facebook && (
            <li className="basis-[100px] flex-shrink">
